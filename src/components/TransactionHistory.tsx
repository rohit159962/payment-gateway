    ...
    <div className="space-y-2.5">
      {transactions.map((tx) => {
        ...
        return (
          <button
            key={tx.id}
            onClick={() => setSelectedId((prev) => (prev === tx.id ? null : tx.id))}
            className={[
          ...
          >
            <div className="flex justify-between items-center mb-1.5">
              ...
              <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[10px] font-bold tracking-wide ${s.bg} ${s.text}`}>
